...
Stdlib-only helper to compute empirical additive Fourier coefficients for Collatz per-class statistics.

Input:
  - CSV produced by collatz_c9_2_sampling.py:
      class, weighted_count_A, weighted_count_A_and_B, hat_p, raw_count_A, raw_count_A_and_B

Outputs:
  - JSON summary with per-v2 bucket stats and top modes
  - CSV of modes (xi, v2_xi, absF, reF, imF)

Usage:
  python3 scripts/collatz_c9_2_fourier.py --csv scripts/out/c9_2_M16_N100000.csv --M 16 --output scripts/out
"""
...
def main() -> int:
    ...
    summary = {
        ...
        "top_modes": top_modes,
        "notes": "Empirical diagnostic for Collatz lane: F(xi) computed from g(a)=hat_p(a)-mean_hat_p over classes present in CSV.",
    }
