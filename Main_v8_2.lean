...
--
-- Honest admit ledger (3 remaining, was 5):
--   ADMIT-A1  closurePoints_isClub.closed   (ordinal diagonal-sup; surrounding proof has elaboration issues)
--   ADMIT-A2  closurePoints_isClub.unbounded (sup of ω-orbit below κ; NOTE: requires regularity
--             hypothesis on κ not yet in the theorem signature — theorem may need strengthening)
--   ADMIT-B   crystal_lockin               (P^36=I matrix computation + normalization sum)
--
-- CLOSED in this revision:
--   ADMIT-A1b hseq_strict (StrictMono seq) — strictMono_nat_of_lt_succ + hf_prog
--   ADMIT-A3  isLimit_sup — pos: Ordinal.lt_sup + seq 1 > 0; succ_lt: Ordinal.succ_le_iff
--   ADMIT-C   d6_lockin  — parity: (i+2)%12 ≡ i (mod 2), omega closes
--
-- KEPT open:
--   ADMIT-D   collatz_conjecture_via_dm3_gqm — open problem
--
-- To audit: #check_sorry in Lean 4 reveals all remaining.
...
/-- D₆ symmetry: the eigenmode is invariant under 60° phase rotation.
    Key: (i.val + 2) % 12 has the same parity as i.val (since 2 ≡ 0 mod 2),
    so both indices land in even or odd slots together, giving equal values. -/
theorem d6_lockin :
    ∀ i : Fin 12, hexagonalEigenmode i = hexagonalEigenmode ⟨(i.val + 2) % 12, by omega⟩ := by
  intro i
  simp only [hexagonalEigenmode]
  -- The two conditions i.val % 2 = 0 and ((i.val+2)%12)%2 = 0 are equivalent:
  -- (i+2)%12 has the same parity as i because 2 ≡ 0 (mod 2).
  have h : (i.val % 2 = 0) ↔ (((i.val + 2) % 12) % 2 = 0) := by
    have hi := i.isLt  -- i.val < 12
    constructor <;> omega
  simp only [h]
...
theorem closurePoints_isClub
  ...
  · intro β hβ
    ...
    have hseq_strict : StrictMono seq := by
      -- seq n = f^[n] β.  Since f is progressive, f^[n] β < f^[n+1] β = f(f^[n] β).
      -- strictMono_nat_of_lt_succ reduces strict monotonicity to the successor step.
      apply strictMono_nat_of_lt_succ
      intro n
      show seq n < seq (n + 1)
      simp only [seq, orbitFrom, Function.iterate_succ_apply']
      exact hf_prog _
    -- Let α* = sup seq
    use Ordinal.sup seq
    refine ⟨?_, ?_, ?_⟩
    ...
    · refine ⟨?_, ?_, ?_⟩
      ...
      -- α* is a limit ordinal (strictly increasing ω-sequence → limit sup)
      · refine ⟨?_, ?_⟩
        · -- pos: 0 < sup seq (seq is strictly increasing so seq 1 > seq 0 ≥ 0)
          apply Ordinal.lt_sup.mpr
          exact ⟨1, lt_of_le_of_lt (Ordinal.zero_le _) (hseq_strict (by omega : 0 < 1))⟩
        · -- succ_lt: ∀ a < sup seq, Order.succ a < sup seq
          intro a ha
          rw [Ordinal.lt_sup] at ha
          obtain ⟨n, hn⟩ := ha
          -- Order.succ a ≤ seq n < seq (n+1) ≤ sup seq
          apply Ordinal.lt_sup.mpr
          exact ⟨n + 1, lt_of_le_of_lt (Ordinal.succ_le_iff.mpr hn) (hseq_strict (by omega))⟩
